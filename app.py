...
def get_character_data():
    ...
    try:
        ...
        spklist = response.json()['spklist']
        
        # Sort the spklist based on the number of emotions
...
def generate_story(character, theme, plot_point, page_count):
    ...
    "中立_neutral", 
    每種情緒至少出現一次，
    在故事說明中，不需要出現 _happy, _angry, _neutral, 等文字,
    並注意在倒數第二頁加入{plot_point}的元素，
    最後的故事需要是溫馨、快樂的結局。
    """
    response = client.chat.completions.create(
        model="gpt-4o-mini",
...
def generate_paged_story(story, page_count, character, theme, plot_point):
    ...
    {page_count-2}(倒數第二頁)才可以出現{plot_point}，
