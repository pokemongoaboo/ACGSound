...
def generate_story(character, theme, plot_point, page_count):
    ...
    最後的故事需要是溫馨、快樂的結局。
    請在故事中包含以下情緒的段落：
    "中立_neutral", "开心_happy", "疑问_question", "生气_angry"
    每種情緒至少出現一次。
    """
    response = client.chat.completions.create(
        model="gpt-4o-mini",
...
def main():
    ...
    if character_data:
        ...
        selected_character = st.selectbox("选择故事朗读角色", characters)

        # 选择或输入主角
        story_character = st.selectbox("选择或输入绘本主角:", characters + ["其他"])
            ...
            try:
                ...
                for i, page in enumerate(pages, 1):
                    ...
                    with st.spinner(f"正在生成第 {i} 页的语音..."):
                        speech_result = generate_speech(selected_character, emotion, text)
